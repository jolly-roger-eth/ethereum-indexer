...
'ethereum-indexer-db-utils': patch
'ethereum-indexer-fs': patch
...
- The predicate now lives once, in `ethereum-indexer` as `isBigIntLiteral` (with `bnReplacer` / `bnReviver` beside it), and every live copy uses it: the CLI, both browser adapters (including `keepStateOnIndexedDB`, the in-browser path ADR-0002 calls primary), the fs adapter and `db-utils`. This closes the CLI's `TODO share with db-utils`. A dead copy in `ethereum-indexer-js-processor`'s `history.ts` was deleted.
...
The suffix convention itself is still a guess: it cannot distinguish a real BigInt from a contract-emitted string that reads like one. `@ethereum-indexer/processor-sqlite`'s tagged `{__bigint__: "..."}` codec is the form that can, and is where the remaining adapters should go.
