...
'ethereum-indexer-utils': patch
---

Declare the packages the published types import, so installing them actually typechecks.

A type-only import is erased from the emitted `.js` but survives in the emitted `.d.ts`. These packages name types from `abitype`, `eip-1193` and `ethereum-indexer` in their public declarations while listing those as `devDependencies`, so a consumer installing them got declaration files importing packages that were never installed.

Moved to `dependencies`: `abitype` and `eip-1193` in `ethereum-indexer`, `eip-1193` in `ethereum-indexer-browser` and `ethereum-indexer-server`, and `ethereum-indexer` in `ethereum-indexer-utils`.
...
A test now asserts, for every package in the workspace, that each bare specifier in its built `.d.ts` files is a declared dependency. It found the `ethereum-indexer-utils` case, which a search for the two known package names had missed.
