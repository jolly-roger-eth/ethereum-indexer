...
- **MutationContext** — the write surface a handler gets for ONE block (`get` / `set` / `delete`, plus `update` as sugar over get-then-spread-then-set), with read-your-writes inside that block. Uniformly async on every backend. It carries exactly one SET read, the **listing** below.
...
- **BlockUnavailableError** — the family of "this store cannot answer about that block", ADR-0015 and ADR-0019. `NoSuchBlockError` (the address resolves to no recorded block) and `BlockNotRetainedError` (the block is fine, the versions are outside retention) are its two members. Neither is ever an empty result or a tip read: an as-of read served from the tip is a plausible wrong number nothing downstream can tell apart from a true one.
