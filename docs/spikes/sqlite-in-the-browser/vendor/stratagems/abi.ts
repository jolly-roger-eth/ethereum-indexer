 ...
 * They do NOT exist on the Base GemsGenerator, which is an earlier version, so
 * the three handlers that consume them CANNOT fire on this fixture. They are
 * kept in the ABI so the ported handler bodies stay verbatim and the port
 * covers all thirteen handlers rather than only the ten that can run.
 */
