...
- **A store reports what it PROVIDES, never what it was asked for.** Neither shipped store prunes, so a configured window is validated and then reported as `unbounded`, which is what those stores actually do. Understating retention is safe (a caller relies on less history than exists); claiming a window nothing enforces is the exact failure the report exists to prevent. `prune-versions-outside-retention-window` earns the right to report a window.
