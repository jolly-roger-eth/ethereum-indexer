...
| `fixtures/*.trace.json` | the per-block mutations the port emitted. NOT committed: `run/verify-port.ts` regenerates them identically in about three seconds |
...
| `src/port/`, `vendor/stratagems/` | the port, and the oracle copied verbatim |

**Licence note, settled:** the vendored stratagems source is GPL-3.0 and this repository is MIT. Both are the same author's work and using it here as an example and test fixture is fine; the note exists so the difference is visible to a reader who meets the code without this context, and it is repeated in `docs/spikes/sqlite-in-the-browser/vendor/stratagems/README.md`.
