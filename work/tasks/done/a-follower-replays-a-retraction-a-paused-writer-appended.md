...
- **The question that replaced the cursor comparison: the stream's own EMISSIONS over the range the follower resumes from, not any cursor.** A follower now keeps `followedEmissions` (block hash + index in block + applied/retracted, in stream order, the identity `sameEvent` already defines) and returns early only while what `fetchFrom` hands back over that range is that exact list. **Why it cannot go stale the way `lastToBlock` did:** it is not a claim the writer makes *about* the stream that some other mechanism can freeze — it is the content of the stream over precisely the bytes the next replay would consume, so any append there differs whatever a cursor says. Its failure direction is the safe one: it is recorded only where this generation has just folded that exact slice, so "unknown" means fold, and an unnecessary replay de-duplicates against the unconfirmed window and delivers nothing. The stored cursor is still consulted for the one half it cannot be wrong about (a stream reaching *past* this fold is new by definition), which keeps the cheap integer test first. **Alternatives considered and rejected:** add `latestBlock`/`lastFromBlock` to the cursor comparison (they move for reasons unrelated to appends, so an idle follower would re-walk every cycle — a performance bug for a correctness one, and the same summary-can-lie mistake in a new place); delete the early return (correct but re-decodes and re-walks the window forever); have the writer publish an append counter or stream digest (a new field on a cursor contract ADR-0035 deliberately keeps to four properties, maintained by every keeper, and still a summary); make a paused writer move `lastToBlock` (that cursor is the drain's termination condition — explicitly out of bounds). **What it touches:** only the follow path (`promiseToFollow`); `indexMore`, pause/resume, the one-writer rule and the read-only stream view are untouched, and there is no new flag, config key or public API. Recorded as **ADR-0049** (`docs/adr/0049-a-follower-asks-the-stream-what-changed-never-its-cursor.md`), because a cursor comparison is the obvious thing a future optimiser will reintroduce here — which is exactly how this bug arose.
- **The memory is discarded on reconfigure and processor swap, and in `forgetStoredStream`.** A reconfigure can point the keeper at a different stream, so what was folded no longer describes what will be read. Forgetting costs one replay that de-duplicates to nothing; keeping it could skip a real append. Touches `updateIndexer` / `updateProcessor` / `reset` only in that they now also clear this field.
- **I amended one clause of ADR-0045** (its final bullet said this hole was open and recorded in `work/notes/observations/`) to point at ADR-0049 instead. Alternative was to leave 0045 untouched and let it contradict the code; a stale "this is still open" line in an accepted ADR is exactly the kind of drift the next reader trusts. No decision in 0045 was changed.
