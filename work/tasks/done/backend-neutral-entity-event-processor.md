...
**The sync cursor's home is the storage seam, as an opaque string, written with its block.** Chosen (the task decided it; building it confirmed it). Alternatives considered and rejected: a new narrow keeper interface the deployment wires (a second thing to misconfigure, and *no atomicity*, which is the whole point); a reserved entity through the existing seam (the cursor would become versioned, revertible and prunable — destroyed by exactly the operations it must survive); making replay idempotent instead of atomic (weakens a check that catches a real caller bug). Touches: every `StateStore` implementation now and forever, `@etherfold/processor-sqlite`'s public surface, and `bootstrap-an-entity-store-from-a-snapshot`. Recorded as **ADR-0027**.

**The cursor is written per BLOCK, not per `process` call.** The task said "the same transaction as the block it describes", which literally reads either way. Writing only with the last block would close the narrow window the observation described and leave the wider one open: a crash after block 3 of 5 still puts state ahead of the cursor and still wedges. So `applyEventStream` derives an intermediate cursor per block (`syncedThrough`: truncate `lastToBlock` and the unconfirmed window to that block; leave `latestBlock` and `context` alone) and gives the last block the stream's own `lastSync`, which is safe because the stream covered every block up to `lastToBlock`. Alternative rejected: making the whole stream plus cursor one transaction, which needs a batched seam verb and would force an unbounded backfill into a single request. Cost: one extra serialization per block, small in practice because `unconfirmedBlocks` is empty during backfill and streams near the tip are short. Touches: `applyEventStream`'s signature (the cursor argument is optional, so existing callers are unaffected) and any future processor over the seam.

**COHERENCE: the port's argument is a `key`, not a "context key".** The task's wording was "under a context key", but `context` already means `ContextIdentifier` in this repo (source/config/processor hashes, validated by the core). Keying the cursor by *that* would break the core's discard path, which only runs when `load` returns something — a context-keyed cursor answers "nothing stored" after a processor upgrade and the new run indexes on top of the old processor's rows. So the seam's key names *which* cursor, the neutral processor writes one fixed `SYNC_CURSOR_KEY = 'lastSync'`, and the context travels inside the value. Both `cursor.ts` files say so explicitly, and the glossary carries the distinction. Touches: anyone later tempted to make the cursor multi-tenant.

**SQLite's cursor table is `_cursor (key, value)`, not the old `_sync (id, lastSync)`.** A storage primitive should not have a column called `lastSync`. Keeping the old shape would have made existing databases resume, which I weighed and dropped: nothing is published under `@etherfold` yet (all packages are `0.0.0`, and `publish-etherfold-and-deprecate-old-names` is still in the backlog), and the spec puts migrating existing processors out of scope. An existing database reads as never-synced and re-indexes. Touches: `@etherfold/processor-sqlite`'s public surface — `SYNC_TABLE`, `SYNC_ROW_ID`, `SYNC_SCHEMA_DDL`, `readLastSync`, `writeLastSyncStatement` and `deleteLastSyncStatement` are **removed** (the changeset names them); `serializeLastSync` / `deserializeLastSync` survive as re-exports. Two assertions in `processor-sqlite`'s tests were retargeted from `_sync` to `_cursor`, meaning unchanged.

**`assertProcessorVersion` is called in both the wrapper and the inner processor.** Not redundancy: the message names the class the caller actually constructed, and a deployment told to fix `EntityEventProcessor` would be hunting a class it never wrote. Touches: `processor-sqlite/test/version.test.ts`, which asserts exactly that.

**The cursor is not reverted and not pruned.** `revertTo` leaves it alone on every backend — how far the *caller* got is not entity state, and the caller moves it when it applies the canonical branch, which arrives in the same stream as the retraction. Asserted in the conformance suite so a backend cannot quietly decide otherwise.

**IndexedDB's schema version moved 1 → 2.** `keys.ts` previously said the schema "stays version 1 forever". That claim was about *declarations* (a processor gaining an entity is still not a migration and still cannot be blocked by another tab), and I rewrote the comment to say so rather than deleting it. The upgrade is additive and `contains`-guarded. Touches: any existing browser database, which gains `cursors` and keeps every row.
