...
- **The reported field's SHAPE is an envelope: `cursor: {reported: true, value: <verbatim>}` / `{reported: false, reason}`, absent entirely when no reporter is injected.** Chosen over a bare scalar (`cursor: 4242` or the opaque string) and over a server-parsed shape (`{lastToBlock, latestBlock}`). A scalar could not grow the generation dimension without changing its type under readers, and the raw string is a serialized `LastSync` carrying an unconfirmed window of decoded events. A parsed shape would give `@etherfold/server` a cursor's meaning that ADR-0027 keeps behind the seam. **How the generation dimension grows inside it:** a later host that holds generations adds a key BESIDE `value` (`{reported: true, value: <the canonical generation's report>, generations: {...}}`), so `value` keeps meaning "the generation answering reads" and no existing reader breaks. Touches: the `/status` response body every client and dashboard reads, `the-server-and-cli-hold-generations-too`, and whichever CLI command (`run` / `build` / `index`) first builds a reporter. This is a published response body, so it met the ADR gate: `docs/adr/0047-the-status-cursor-is-a-reported-envelope-the-server-never-parses.md`.
- **Degraded means absent-WITH-A-REASON rather than an omitted field.** The criterion allowed either. A reason is what lets an operator tell "this deployment reports no cursor" (a Worker, a `serve` tier) from "this deployment's reporter is broken", which is the distinction the status page exists to make. Omission is reserved for the one honest absence: no reporter injected.
- **The server calls `JSON.stringify` on the report inside the degrade path.** An unserialisable report (a `bigint`, which is what every decoded `uint256` here is) would otherwise throw inside `c.json`, after the guarded code has returned, and answer `500` on the page an operator watches while something is wrong, which is exactly the failure the "a reporter must not take `/status` down" constraint forbids. Alternative considered: rely on the type alone, since `CursorReport` is JSON-shaped, and accept the runtime hole for a host that casts. The probe asks whether the value can be SENT, never what it means, so it is not parsing. Touches nothing outside this route.
- **The option is named `getCursorReport`, not `getCursor`, and it may be async.** `sync cursor` is a defined glossary term meaning the opaque string, and an option named `getCursor` whose documentation forbids returning the cursor would re-mean it; a reporter returns a REPORT about the cursor. Async because reading a cursor is a store read, unlike `getDB`/`getEnv`/`getIngestion`, which hand back handles a host already holds. The response FIELD is still named `cursor`, which is the name `CONTEXT.md` and the spec already promise.
- **`StatusCursor.value` is typed `unknown` while the reporter's return is the recursive JSON `CursorReport`.** Not a stylistic choice: the response type reaches the Hono RPC client type through `c.json`, and the recursive type made the compiler give up with `TS2589` (`hc<App>('')` in `packages/server/src/index.ts`). `unknown` is also what the server actually knows, and the serialisability obligation still binds where a host writes its reporter, which is the only place that can honour it. Touches any consumer of the typed client, which now sees `cursor.value` as `unknown` and must narrow it itself.
- **A failing reporter is logged but does NOT set `lastError`.** It follows the reorg counters' precedent in the same route (their read failure is swallowed), and the reason is already reported in the field itself; `lastError` stays "the error this process saw" about the server's own work rather than about a host's injected callback.
