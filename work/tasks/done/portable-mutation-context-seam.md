...
- **The seam is TWO packages, not one: `@etherfold/state-store` (contract) and `@etherfold/processor-entities` (ABI-typed authoring).** Chosen because the `on<EventName>` map needs `LogEvent`/abitype from `@etherfold/core`, and core depends on viem. A single package would force `@etherfold/state-store-sqlite` either to depend transitively on the whole indexer core (inverting ADR-0016's stated direction and its "imports nothing but remote-sql and named-logs" test) or to keep a duplicate, structurally-compatible copy of `EntityDeclaration`/`Mutation`/`BlockPointer`. Both alternatives were considered and rejected; the split keeps every definition singular and the store a primitive. Touches: ADR-0016's dependency assertion, `state-store-sqlite`'s leakage test, and every future backend package (`light-store-behind-the-seam`, `indexeddb-row-backend-browser-default`) which now depends on `@etherfold/state-store` alone. Recorded as **ADR-0018** (`docs/adr/0018-the-storage-seam-is-two-packages.md`).
- **Names follow ADR-0014's role-first axis with the trailing slot filled by a non-engine.** `state-store` = the role with no backend, i.e. the contract; `processor-entities` = a processor whose state lives in declared entities. `entity-processor` and `processor-api` were rejected for breaking the ordering rule that keeps one role's variants adjacent in a scope listing.
- **What moved vs. what stayed.** Moved to the seam: entity declaration + `FieldType` + `normalizeEntity`/`idValues`, `EntityId`, `Mutation`, `BlockPointer`/`BlockUpdate`, `normalizeBlockHash`/`normalizeBlockTimestamp`, `MutationContext` + staging, and (to `processor-entities`) the handler types, `forkPoint`, `groupByBlock`, `blockPointer`, and the revert-then-apply loop. Stayed SQL-specific: `Statement`, DDL, batching, `queryCurrent`/`queryAsOf` (caller-supplied SQL), block addressing by hash/timestamp, `NoSuchBlockError`, and the `_sync` cursor. Every moved symbol is re-exported from its old package, so no existing import broke.
- **`StateStore.getAsOf` takes a resolved block NUMBER, not a `BlockAddress`.** This keeps `NoSuchBlockError` from having to be duplicated (two classes of one name would break `instanceof` across packages, ADR-0015's family) and leaves `retention-capability-and-refusal` free to decide where the refusal family lives. Alternative considered: moving addressing to the seam now — rejected as out of scope and premature. Touches `retention-capability-and-refusal` and `state-store-conformance-suite`.
- **The interface is named `StateStore`, not `StateBackend`.** The spec's prose says "backend"; ADR-0014 already uses "backend" to mean the storage ENGINE named in a package (`sqlite`), so naming the role-level interface `StateStore` avoids re-meaning it. Prose keeps "backend" for the engine.
- **`MemoryStateStore` ships inside the contract package** rather than as a third `state-store-memory` package: it is the contract's executable definition and the second backend a portability test needs, not a production backend.
- **`update` was added to `MutationContext`** (spec decision 3 and acceptance criterion 4 require it; it did not exist). It strips to declared fields before spreading, so a backend's version columns cannot travel back in as values — pinned by a test on both backends.
- **`ColumnType` → `FieldType`,** with `ColumnType` kept as a deprecated alias in `state-store-sqlite`: the four values are the intersection of what all backends can hold, not SQLite storage classes, and the declaration already calls them `fields`. Touches `tagged-bigint-codec-across-storage-adapters`, which now has a backend-neutral type to evolve.
- **Identifier validation moved to the seam and now applies to every backend.** This makes "valid declaration" a fact about the declaration rather than the deployment, which is half the fix for the finding's `index`-keyword defect; the other half remains `entity-identifier-sql-keyword` (unchanged, untouched).
