...
- **`processor-version-hash-cannot-silently-lie` (in `tasks/ready/`) predated this task and was scoped to the JS path only.** There are now two `EventProcessor` implementations; this one already folds the entity schema into its hash (decision 5) and already persists its context in `_sync` — exactly the "where the fingerprint is persisted" question its own prompt says to re-check. **Re-scoped on 2026-08-21** to cover both implementations, so it is ready to build as written. Note that this package reproduced the very `unknown` fallback that task exists to remove, which is what a single-implementation scope buys you. **Landed in `33afc5b`**; the record now rests in `tasks/done/`, and decision 5 of this task (hashing the entity declarations) survives inside the reworked hash.
