...
- **The flag names were NOT invented here.** `--ingest-endpoint` / `--ingest-token` (with `INGEST_ENDPOINT` / `INGEST_TOKEN` behind them), `-n`, `-d`, `--rps` and the whole `fetch` row already existed in `packages/cli/src/config.ts`, landed by `one-configuration-path-for-every-command` and already asserted in `configuration.test.ts`. So this command CONSUMES that table rather than extending it: no new input name, no second way to read a flag. The only thing I added on that axis is passing `--rps` through as `requestsPerSecond`, since the table says `fetch` owns it. Alternative considered and rejected: a shorter `--endpoint` / `--token`, which would have been a second name for a variable that is already the published contract of a deployable.
- **`fetchMain` re-uses the adapter's `runFetcherProcess` WHOLESALE, so the adapter owns the exit code; `fetch()` uses only `startFetcher`.** The criterion is "the same exit code the retired binary did", and the cheapest way to be the same is to be the same function: `0` when asked to stop, `1` on a refusal no waiting fixes. The CLI adds only the half `runFetcherProcess` cannot see — a CONFIGURATION refusal raised before a loop exists — which it catches and exits `1` on, as every other refusal in this CLI does. The alternative (drive `startFetcher` and map `summary.stoppedBecause` in the CLI) would have put a second copy of that mapping one package away from the first, and it is the mapping most likely to drift silently. Cost, accepted: `runFetcherProcess` takes no `AbortSignal`, so the "exits 0" test stops it with a real `SIGTERM` (as `platforms/nodejs-fetcher/test/loop.test.ts` already does) rather than an injected signal. Touches: `platforms/nodejs-fetcher`'s exit-code contract is now depended on by `etherfold fetch`.
- **The console log sink is installed in `program.ts`'s default `fetch` handler, not globally in `cli.ts` and not at module scope.** It has to be at a PROCESS entry point (a library must not choose an embedder's sink), and the CLI is the process now. Module scope in `fetch.ts` was rejected because `etherfold` is also a library (`index.ts` re-exports `fetch`), so importing `named-logs-console` there would hook the console for anyone importing the package. `cli.ts` (global, for every command) was rejected as OUT OF SCOPE and user-visible: it would change what `run`, `build` and `serve` print. Consequence a later task may want to settle: `run` also drives a fetch loop and its `named-logs` lines still reach no sink, so the observability of the two commands is now asymmetric. Touches: `run`/`build`/`serve` output, and `ProgramDependencies.fetch` (a test substituting the handler installs no sink, which is why `commands.test.ts` stays silent).
- **The CLI's exported function is named `fetch`**, matching `run` / `serve` / `build` (named for the command). It shadows the global `fetch` inside `src/fetch.ts` (where the global is never called: the wire's `fetch` is passed through as a dependency) and for a consumer who imports it by that name. Alternative considered: `startFetching`, rejected because it reads as a near-duplicate of the adapter's `startFetcher` and breaks the one-word-per-command pattern.
- **Two changeset files rather than one covering both packages.** The criterion asks for "both changesets", and the two audiences differ: a `@etherfold/platform-nodejs-fetcher` reader needs "your binary is gone, here is what replaces it" at the top, an `etherfold` reader needs the new command. The repo has both patterns (`the-old-indexer-shape-is-deleted` bundles; most others do not).
- **`@etherfold/server` added as a `devDependency` of `etherfold`.** The loop test needs the REAL receiver on the other end of the wire (a mocked one would mock the thing under test: the cursor and the `409` live there), following `packages/fetcher-host/test/harness.ts`. Test-only, so it does not change what the CLI ships. `ldenv` and `named-logs-console` were removed from the fetcher platform's runtime dependencies, since only the deleted entry point used them.
- **`CONTEXT.md`'s command-set bullet was corrected, which also fixed drift I did not create.** It said three of the five commands "do not exist at all", naming `run` among them — already false before this task (`run-follows-folds-and-answers-in-one-process` did not update it), and my change makes it falser. Since that bullet is explicitly the thing a reader is told to land on, I corrected the whole sentence rather than only my half.
