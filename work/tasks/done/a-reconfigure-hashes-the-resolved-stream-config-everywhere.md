...
**Where the single resolve-then-hash step lives: `streamConfigHashOf` in `packages/core/src/internal/engine/utils.ts`, immediately beside `resolveStreamConfig`.** Chosen because that file already owns the other two questions this one is an instance of (`resolveStreamConfig`, `wireContextOf`), and because both halves of a split deployment already import their identity rules from there, so the sender and the receiver cannot reach two answers. Alternatives considered: `utils/hash.ts` (rejected — those bytes are the shared, persisted canonicalisation and a config-aware helper does not belong beside them), and a private method on `IndexerGeneration` (rejected — it would be unreachable from `captureStream` and `wireContextOf`, which is precisely how the split happened). The name is not a new concept: it names the existing `streamConfigHash` field / `ContextIdentifier.config` value, follows the repo's `...Of` convention, and adds nothing to the glossary.

**I exported `streamConfigHashOf` from `@etherfold/core` (a public-surface change, hence the changeset).** This is what keeps a *third* caller from reintroducing the split: a host or a test building a `ContextIdentifier` or `WireContext` of its own must reach the same digest the engine stored, and the structural test only guards `packages/core/src`. The rationale is `resolveStreamConfig`'s own, one step further along. Alternative: keep it internal (rejected — it leaves every cross-package caller re-deriving `simple_hash(resolveStreamConfig(x))` by hand, which is the exact expression that drifted). Touches: the published surface of `@etherfold/core`; nothing else changes call shape.

**I routed `wireContextOf` and `captureStream` through it too, not just the two named call sites.** `captureStream` was a genuine THIRD instance of the same defect — it hashed `options.streamConfig ?? {}` as provided, so a fixture captured with `finality` unset wrote a `config` digest no indexer running the resolved default could match, and the load path would read the replayed stream as belonging to another config and clear it. Making the acceptance criterion "no second site hashes a `ProvidedStreamConfig`" *true* required fixing it. Consequence to be aware of: a NEWLY captured fixture whose `streamConfig` omits `finality` now records the resolved digest instead of the unresolved one. No committed fixture moves — nothing is regenerated, and the one committed stream fixture (`stratagems-base.stream.json`) was captured with `{finality: 12}` spelled out, so its digest is identical either way. `wireContextOf` takes a `UsedStreamConfig` and so was already correct; it goes through the step for the structural rule rather than for a behaviour change, and idempotence is asserted so the wire keeps its bytes.

**The guard is a source scan, in the house style of `streamIdentity.test.ts`.** A comment does not satisfy the criterion, and a purely semantic test cannot see a second site that happens to agree today. The scan asserts the *set* of files that hash a config equals exactly one. Its known limit: it matches on `simple_hash(` textually, so an aliased import would slip past it — which is why the companion assertion is that `indexer.ts` contains no reference to `simple_hash` at all.
