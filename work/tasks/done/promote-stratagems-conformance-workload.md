...
**Where the promoted artifacts live: a new private workspace package, `packages/conformance-workload-stratagems/`, with the fixtures and the vendored oracle MOVED (not copied) out of `docs/spikes/`.** `docs/spikes/` is an evidence store for a finished measurement; a conformance fixture is production test material that must not rot. A copy would have meant 1.3 MB duplicated and two goldens that can silently diverge. The spike's `src/port/` deliberately STAYS behind, because it is the exhibit the finding's contortion list describes, and the promoted port is a rewrite of it: the spike README, the finding and the spike's scripts all now point at the new home. Alternatives considered: inside `@etherfold/state-store-conformance` (refused: `processor-entities` closes a workspace dependency cycle, and the licence), or `examples/` (refused by the spec: a workload is not a teaching example). Touches: `@etherfold/state-store-conformance`'s README and doc comment (hence the changeset), and every spike script that read a fixture. Recorded as **ADR-0026**.

**The fast/full split.** Fast, every invocation (about 2s): the `stratagems-base` smoke replay on all FOUR backends, the model-shape regression guard, and the oracle re-deriving BOTH golden states. Full (`CI` set, or `test:full`, about 18s): the launched game's 31,332 events on memory, sqlite and patch, plus the named revert case. Opt-in only (`test:all-backends`): the launched game on IndexedDB, because on `fake-indexeddb` it costs about half an hour and degrades quadratically, where the same backend measured 45.6 ms/block on real Chromium. So the heavy IndexedDB coverage is a property of the SHIM, not of the backend, and the honest route to it is the real engine. Alternative considered and rejected: putting it in CI anyway (it would be switched off by the next person to wait for it). Touches: any CI workflow that wants the heavy tier must set `CI` (it does) or call `test:full`.

**The oracle now re-derives BOTH golden states on every invocation, not just the small one.** The inherited code justified oracle-on-small-fixture-only with "running it over 31,332 events would pay the oracle's runtime to learn nothing". I measured it: 1.2s, because the original processor is in-memory immer with no store underneath. That justification was false, so the expensive fixture's oracle would have been the one nobody re-ran. `run/regenerate-golden-state.ts` remains a script for the one thing that actually differs: it WRITES.

**`packages/conformance-workload-stratagems/package.json` declares `"license": "GPL-3.0-only"`, the only package here that does not say MIT.** ADR-0026 refuses to publish the derived port under MIT because that would misstate what it is; leaving the manifest at MIT restates exactly that misstatement in the file a reader and every licence scanner check first. It costs nothing (the package is `private`), and the odd field is one click from `vendor/stratagems/README.md`. Alternative considered: leave it MIT and rely on prose (rejected: the prose is what the manifest contradicts). Touches: nothing in the release path, since changesets and `pnpm publish` skip private packages.
