...
**1. The commander wiring moved into `src/program.ts` behind `createProgram({env, build, serve})`, with the handlers injectable.** The task names "the command surface" as a seam to test at, and `cli.ts` could not be one: it called `program.parse(process.argv)` at import and read `process.env.ETHEREUM_NODE` at registration time, so importing it ran the CLI. Injecting the two handlers is the same shape `main(options, {run, exit, log, error})` already uses in this package, so asserting which WORDS resolve and which flags they carry costs no processor module and no bound port. Alternative considered: spawn `node dist/cli.js` from the test, which asserts the real binary but makes the CLI suite depend on a prior `pnpm build` (the package's own `pnpm --filter etherfold test` would then be red on a clean clone). **Touches**: nothing outside the package — `createProgram` is not re-exported from the package entry, and the next three command tasks (`run`, `fetch`, `index`) register into this same function.

**2. The exported `run(options)` is renamed `build(options)`, and `main`'s injectable `run` collaborator becomes `build`.** Not in the acceptance criteria, and I judged it inside the task's stated goal ("so the three commands that do not exist yet can be added without a word being reused") rather than outside it: `run` is a name in the five-name set, it means a process that follows the chain, answers queries and NEVER terminates, and `etherfold`'s package entry was exporting a `run` that folds to the tip and returns. `run-follows-folds-and-answers-in-one-process` is the very next task and would have had to either pick a different name for its own command's function or re-mean this one. Alternative considered: leave it and note the collision for that task to resolve, which pays the rename later at the same price plus a period where the word means two things. **Touches**: the `etherfold` package's public API (in the changeset) and `run-follows-folds-and-answers-in-one-process`, which should now find `run` free.

**3. `resolveIndexOptions`, `prepareIndexing` and `PreparedIndexing.index()` keep their names.** These name the DOMAIN verb — `CONTEXT.md` is explicit that indexing is turning logs into state, and that the `index` COMMAND gets that word precisely because it is the folding half. `index-receives-a-pushed-stream-and-owns-the-database` takes `-p`, `--store`, `--db` and `--retention` too and will resolve them through this same path, and `one-configuration-path-for-every-command` owns unifying it, so renaming to `resolveBuildOptions` now would be undone next. Only the doc comments that named the COMMAND (`the flags etherfold index takes`) were corrected. **Touches**: `one-configuration-path-for-every-command`, which inherits the naming question.

**4. The example's script key is `build:db`, not `build`.** `build` is `tsc` in that package (and `build:db` needs it to have run first), and `browser:build` already exists, so `build:db` reads as "the etherfold build that produces the database" and collides with neither. Alternatives considered: keeping the key `index` (which would leave the example teaching a word that now means the receiver), or renaming `tsc` to `compile` to free `build` (a bigger, unrelated change to a package this task only documents). **Touches**: the root README, the example README and `src/cli.ts`'s JSDoc, all updated in step; anyone with `pnpm --filter event-processor-nfts index` in their fingers.

**5. The `serve` docs say it writes no INDEXED state and name the schema as the one write it does make.** The criterion's words are "writes no state", and the read tier does apply the fixed-table schema at startup unless `--no-auto-setup` — which the task explicitly puts out of scope and tells me to leave exactly as it is. Writing the literal sentence would have made the README false about behaviour the task deliberately preserved, so I documented the promise (no processor, no chain call, no logs received, no indexed state) and the exception (the schema, and the flag that turns it off) rather than choosing between an inaccurate doc and an out-of-scope code change. **Touches**: `one-configuration-path-for-every-command` and any later task that revisits whether a read tier should apply a schema at all.

**6. A bare `etherfold` prints help to stderr and exits 1 — commander's own behaviour, left alone.** With no default command and no action handler on the program, commander already emits full help and a `commander.help` exit. I did not add an explicit action to redirect it to stdout or force exit 0. The criterion is "prints help and exits without indexing anything", which this satisfies, and it matches what `git` with no arguments does. **Touches**: any later task that wants `etherfold` alone to be a success exit would be changing commander's default, not removing something I added.
