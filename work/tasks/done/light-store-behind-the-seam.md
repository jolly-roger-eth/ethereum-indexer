...
**Package name and placement: a new package `@etherfold/state-store-patch`, class `PatchStateStore`.** It cannot live in `@etherfold/state-store`, which declares *no* dependencies (asserted by `state-store-sqlite/test/no-platform-leakage.test.ts`) and this store needs immer. Name read off ADR-0014/0016's axis (role first, backend last), with the trailing slot naming the *representation*: `state-store-memory` was rejected because `MemoryStateStore` already means "in-memory versioned rows" in the seam package, and two "memory" stores would re-mean an existing term. The spec's phrase "the light store" is kept as prose in the README/ADR rather than as a package name, since "light" names a feeling and "patch" names the mechanism. Touches: the naming axis in ADR-0014/0016; the future `indexeddb-row-backend-browser-default` package sits beside it under the same rule.

**Persistence: memory-only, and reload is an empty store.** State and the patch log both go with the process. Alternatives considered: (a) serialise the whole state on save, which is the incumbent `keepStateOnIndexedDB` and belongs to the `KeepState` seam *above* storage; (b) row-level persistence, which is `indexeddb-row-backend-browser-default`'s job and a different backend. Doing either here would fork a persistence strategy inside the backend whose whole claim is that it is the cheap one. Consequence, stated rather than discovered: a host using this store must re-index from a cached stream (or hydrate from a snapshot) on load. Touches: `indexeddb-row-backend-browser-default`, the `KeepState` seam, and whoever wires the browser default. Recorded in ADR-0023.

**`durability: 'memory-only'` is reported on this backend's own capability object, not added to `StateStoreCapabilities`.** The acceptance criterion asks the capability report to say so; adding a field to the shared type would make every backend fill in a constant for one implementation's benefit and would break existing exact-equality assertions on `capabilities` in `processor-entities/test/two-backends.test.ts`. So `PatchStateStoreCapabilities = StateStoreCapabilities & {durability: 'memory-only'}`: structurally still a `StateStore`, and the extra word sits where a caller already looks at startup. Alternative if a second backend ever needs it: promote `durability` to the seam. New named concept, checked against the glossary: it does not overlap `retention` (how far back history reaches) — it is whether anything survives the process.

**A revert deeper than the surviving reverse patches is a typed error (`RevertBeyondPatchHistoryError`), atomic, and NOT a member of `BlockUnavailableError`.** Alternatives: revert as far as possible and report the depth reached (rejected: a half-undone reorg is the write-path twin of a tip read served as history, and keeps exactly the counter a reorged-out block raised), or keep every reversal forever (rejected: that is an unbounded patch log, and it is not what `revert-only` means). Not filed under `BlockUnavailableError` because that family is about a *read* the store cannot answer, where the caller can carry on with the tip; here the state is known-ahead-of-canonical and a `catch` written for a refused history query must not swallow it. Touches: any host wiring this backend needs a re-index path — `applyEventStream` in `@etherfold/processor-entities` will surface it from `revertTo`, so `one-processor-cli-and-split-server` and the browser wiring inherit it. Recorded in ADR-0023, with `retainedReversals()` exposed so a host can see the real depth (one block on a sparse stream) before it needs it.

**`retention` is accepted only as `'revert-only'`; a window or `'unbounded'` throws at construction.** The seam's doc says a store reports what it *does* rather than what it was told, which would suggest downgrading silently; I refuse instead, because a deployment that wrote `{blocks: 128}` believes it can read history and should find out where it configured it. Alternative considered: accept and report `revert-only` (rejected as a quiet demotion). This introduces a new refusal, so it is recorded here; it touches how a generic deployment passes one retention spelling to any backend.

**`PruneReport.versionsDeleted` counts blocks whose reversals were dropped.** This backend has no versions; one block's reverse patches are its unit of history, and `maxVersions` budgets the same unit. Documented at the method. Touches the shared `PruneReport` type's reading on a non-versioned backend.

**The cross-backend equality test lives in `processor-entities/test/`, not in the new package.** That is where "one processor, several backends" already lives, and it keeps the store package free of a devDependency on the indexer core (viem), preserving ADR-0016's direction even in the test graph.
