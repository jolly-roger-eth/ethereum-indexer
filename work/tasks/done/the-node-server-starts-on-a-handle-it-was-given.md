...
- **The two start forms are ONE option with a union type (`db?: string | RemoteSQL`), not two keys.** Considered a second key (`db` for the URL, `handle`/`database` for a `RemoteSQL`), which would make the discriminator a name instead of a `typeof`. Rejected because two keys create a third, meaningless state (both supplied) that then needs a refusal, and because the concept is one thing: a host has ONE database, and the only question is who opened it. A union keeps `db` meaning exactly what it means today and what `RunningServer.db` already returns, so a reader meets one word for one concept. Touches: any caller of `startServer` (today `packages/cli`'s `serve`, unchanged since a string is still accepted) and the CLI commands `run` / `index` that will pass a handle.
- **The ingestion and the reporter are accepted in the SERVER's own per-request shape (`(c) => ...`), not as plain values.** Considered accepting a `LogIngestion` value (and a plain report function) and wrapping it, which reads more naturally for a Node process that builds exactly one ingestion at startup. Rejected because the adapter's job is passthrough: re-shaping would make `platforms/nodejs` a second, subtly different injection surface that has to be kept in step with `ServerOptions`, and it would silently forbid a host that wants to resolve per request (which is the model the app is built on, because on Workers the database binding only exists per request). A caller with a single value writes `() => ingestion`. Consequence to note deliberately: `db` accepts a VALUE while the capabilities accept FUNCTIONS, and that asymmetry is intended — the database is a thing this adapter would otherwise have to build itself from a URL, the capabilities are things it can never build. Touches: `run` / `index` in `one-command-runs-the-whole-pipeline`, which will write `getIngestion: () => ingestion` and `getCursorReport: async () => report`.
- **Typed by indexing `ServerOptions<NodeEnv>` rather than importing `LogIngestion` / `CursorReporter` directly.** `CursorReporter` is exported by `@etherfold/server` and could have been imported by name, but `LogIngestion` lives in `@etherfold/core`, which this package does not depend on and must not start depending on ("it names no store package and no chain concept"). Indexing the options type gives both without a new dependency and makes the passthrough literal in the type: whatever the server accepts is what this accepts. The test spells its fake ingestion the same way, so the test adds no dependency either.
- **`close()` does NOT close the database handle, on either form.** That is today's behaviour for the URL form and it is now load-bearing rather than incidental: a server that closed a handle it did not build would take a store's connection down with it, which is precisely the failure the shared-handle form exists to avoid. Considered closing only handles the adapter itself built (it can tell), and rejected it: it would make shutdown mean two different things depending on how the server was started, and the URL form's handle is returned as `running.db` for exactly the sharing this task enables, so "the adapter built it" is not the same as "nobody else uses it". Stated in the doc comments on `StartOptions.db` and `RunningServer.close` and in the README. Touches: whichever CLI command owns process shutdown; it closes the handle it built, after stopping the server.
