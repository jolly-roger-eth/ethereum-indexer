...
**A stream is addressed HIERARCHICALLY, not by a delimited string.**
`['stream', <indexer-name>, <streamDigest>, <ordinal>]` — ARRAY keys on IndexedDB (`idb-keyval` takes
`IDBValidKey`, which includes arrays) and columns on SQL. Enumeration is then a SCOPED listing rather than a
...
**Segments are immutable and independently readable**, both stated so a test can FAIL them: no write
ever targets a segment's address again, and any segment is readable BY ITS OWN ADDRESS without
reading the others. These are REQUIREMENTS rather than aspirations, because a later design
that wants a second reader over a prefix of this stream has nothing to stand on without them.

## User Stories

1. As a developer doing a long backfill, I want a save to cost its BATCH and not my history, so a
   large history does not slow its own ingestion. (The delivering task is stronger than an earlier
   draft of this story, which allowed "one tail plus its batch": with one segment per batch and no
   open tail, nothing already written is rewritten at all.)
...
7. As a maintainer, I want a segment never rewritten and readable on its own, so a later design can
   refer to a prefix instead of copying it, without this spec having to build that.
...
Recorded in an observation since discharged by this spec and deleted.
