...
schema version and the last error. It does NOT report the cursor: the word does not occur anywhere in
`packages/server/src`. That matters twice over, because the cursor is the field that makes a pipeline
...
**How the stories cut, so a tasker does not emit fiction for three of them.** Stories 1, 3, 5 and 6
are vertical tasks. Story 2 is an ASSERTION on story 1's task (the same processor object under `run`
and in a tab), not a task of its own. Story 4 is nearly delivered already — today's `etherfold serve`
is the read tier — so it cuts as rename plus the refuses-to-write assertion. Story 7 is this spec's
framing and is delivered by the other six; there is no naming task.
