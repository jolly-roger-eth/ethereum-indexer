...
1. As a developer, I want ONE command that follows a chain, folds into SQLite and serves HTTP, so I can
   run the whole indexer locally without assembling components or reading about a split.
2. As a developer, I want the processor object I run in a browser tab to run under that command
   UNCHANGED, so "write it once" is something I can verify from a terminal rather than from a test file.
...
**`serve` is assembly, not new machinery.** It constructs a `LogFetcher`, wires it to a `StreamBuilder`
...
- **The headline is a TERMINAL test, not a unit test**: run `serve` against a fixture chain, assert
...
- **The same processor object under `serve` and in a browser** produces the same state, which is the
...
having no store dependency. `serve` is the first thing that wires a store into a server process, so
