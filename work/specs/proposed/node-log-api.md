...
- **The `logValues` conflict is RESOLVED, by the sibling spec widening its decision to the wire.** Recorded because the resolution is what this API depends on, not merely a note. The hazard was real and structural: `logValues` is applied in `LogEventFetcher.parse`, which in a SPLIT deployment runs on the SENDER, so a projection that drops `topics`/`data` reaches the server already applied and the logs it stores cannot answer `eth_getLogs` at all. `the-stream-stores-only-what-the-node-said` now decides that `logValues` is PROCESSOR-facing only and that storage AND the wire always keep the raw log, which removes the conflict at its source rather than interlocking two configs. The earlier remedy recorded here (the config must refuse to trim while the node API is enabled) is WITHDRAWN: it is the shape that spec explicitly rejected, on the ground that it turns a storage-size knob into a constraint on what handlers can be written. The root `TODO.md` item that raised it can be closed by that change. What this spec must NOT do is re-add an interlock.
