...
- **The `logValues` conflict is CLOSED, by deletion.** Recorded because this API depended on the outcome. The hazard was structural: `logValues` is applied in `LogEventFetcher.parse`, which in a SPLIT deployment runs on the SENDER, so a projection dropping `topics`/`data` reached the server already applied and the logs it stored could not answer `eth_getLogs` at all. `the-stream-stores-only-what-the-node-said` DELETES `logValues` (an unused stub whose only purpose, reducing what is stored, that spec removes), so no configuration can strip the raw log from storage or the wire and the conflict cannot arise. The earlier remedy recorded here (the config must refuse to trim while the node API is enabled) is WITHDRAWN, and so is its successor (make the projection processor-facing): there is no projection to make facing anything. The root `TODO.md` item that raised it is closed by that deletion. This spec must NOT re-add an interlock for a knob that no longer exists.
