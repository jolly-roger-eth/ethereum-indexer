...
1. **Which backend is the recommended browser default?** Provisionally answered already, and the burden of proof now sits on wasm SQLite. `~/dev/github/wighawag/research/browser-embedded-indexer` reached this spec's seam placement independently and recommended, on sourced evidence, running the browser on IndexedDB rather than shipping roughly 839 KB of wasm plus 560 KB of JS that is Worker-only, needs COOP/COEP or a single-connection VFS, and hits `SQLITE_BUSY` across tabs. Its IndexedDB backend is real-browser-verified and adds about 3.3 KB gzipped. What is missing is the head-to-head against OUR workload rather than an assumed one, which `spike-sqlite-in-the-browser` supplies. The seam here is written so the outcome is a configuration choice either way.
...
- **Its workload is the stratagems port, promoted from the spike, not a purpose-built toy.** The spike produces a captured Base stream, a port of a real processor, and the state that port yields; together those are a golden input, a golden output, and an equality oracle that is not our own reimplementation (the existing `JSProcessor` computed the expected state independently). A fixture is a SNAPSHOT by definition, so pinning it to a stratagems commit and block range keeps it honest indefinitely, which is exactly the property a vendored "real world example" would NOT have.
