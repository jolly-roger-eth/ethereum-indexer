...
- The existing HTTP server layer (`packages/ethereum-indexer-server/src/server/simple.ts`) and its
  TODOs (auth / api key, response field shaping) — see `tasks/findings/todo-triage.md`
  ("Server / streams packages"). Relevant to the trigger registration/delivery API surface.
  (Note: the former `ethereum-indexer-streams` package — an un-started multi-source server skeleton
  exposing state-query + event-query APIs over authenticated HTTP — has been removed; its concept is
  superseded by the log-watcher / log-processor split in `plan-historical-state-database.md`.)
