...
| **running it from a terminal** | [`etherfold`](packages/cli) -- `run` follows, folds and answers HTTP in one process; `build` folds to the tip and exits; `fetch` is the chain-facing half of a split deployment; `serve` answers queries over a database written elsewhere |
