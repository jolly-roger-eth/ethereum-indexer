...
import {EthereumIndexer, simple_hash} from 'ethereum-indexer';
import type {AllData, IndexingSource, KeepState, LastSync, LogEvent, ProcessorDriftReport} from 'ethereum-indexer';
...
function memoryKeeper(): KeepState<typeof abi, State, {history: HistoryJSObject}, undefined> & {
	...
	const keeper: any = {
		...
		save: async (_context: unknown, all: AllData<typeof abi, State, {history: HistoryJSObject}>) => {
			...
			// use (`ethereum-indexer-cli`'s bnReplacer/bnReviver, keepStateOnLocalStorage),
