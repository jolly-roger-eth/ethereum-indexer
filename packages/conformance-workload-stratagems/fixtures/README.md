...
- **`*.state.json`** — the golden OUTPUT: the state the **ORIGINAL** stratagems `JSProcessor` computed from that stream (stratagems commit `3d5a0b3f`, 2024-12-18), key-sorted and with bigints in the repo's `"123n"` convention. **A diff on it means the processor changed meaning, and that is a finding, not a fixture to update.**
